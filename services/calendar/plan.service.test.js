...
/** Définir PLAN_SERVICE_INTEGRATION=1 et un utilisateur fake@trainpreddict.fr en base pour exécuter ces tests. */
const maybeDescribe =
    process.env.PLAN_SERVICE_INTEGRATION === '1' ? describe : describe.skip

maybeDescribe('plan.service (intégration PostgreSQL)', () => {
    let plan = null
    ...
    beforeAll(async () => {
        ...
        if (!utilisateur) {
            throw new Error(
                'Fixture manquante : créer un utilisateur fake@trainpreddict.fr en base pour ces tests.'
            )
        }
